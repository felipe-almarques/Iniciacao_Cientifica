...
ll_sv <- function(params, r, fuller=FALSE, c=0.005) {
  ...
  alpha <- params[1]; phi <- params[2]; sigmaw <- params_ini[3]
  sigma1 <- params[4]; sigma2 <- params[5]; mu1 <- params_ini[6]
  mu2 <- params_ini[7]
  ...
  for (t in 1:n) {
    ...
    pi1[t] <- (dnorm(y[t], h[t] + mu1, sqrt(Sigma1[t]))/2) / mean(c(dnorm(y[t], h[t] + mu1, sqrt(Sigma1[t]))/2, dnorm(y[t], h[t] + mu2, sqrt(Sigma2[t]))/2))
    pi2[t] <- 1 - pi1[t]
    ...
    ll_sv <- ll_sv + log(mean(c(dnorm(y[t], h[t+1] + mu1, sqrt(Sigma1[t])), dnorm(y[t], h[t+1] + mu1, sqrt(Sigma1[t]))))) # conferir essa parte
  }
  #h <- h[2:n+1] ; P <- P[2:n+1]
